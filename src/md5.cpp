...
const tbyte MD5::PADDING[64] = { 0x80 };
...
MD5::MD5(const string& message) {
  ...
  init((const tbyte*)message.c_str(), message.length());
}
 ...
 *
 */
void MD5::init(const tbyte* input, size_t len) {

  bit32 i, index, partLen;

  finished = false;

  /* Compute number of tbytes mod 64 */
  index = (bit32)((count[0] >> 3) & 0x3f);
 ...
 */
void MD5::transform(const tbyte block[64]) {
...
/**
* @Encodes input (unsigned long) into output (tbyte).
*
...
*
*/
void MD5::encode(const bit32* input, tbyte* output, size_t length) {

  for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
    output[j]= (tbyte)(input[i] & 0xff);
    output[j + 1] = (tbyte)((input[i] >> 8) & 0xff);
    output[j + 2] = (tbyte)((input[i] >> 16) & 0xff);
    output[j + 3] = (tbyte)((input[i] >> 24) & 0xff);
  }
}

/**
 * @Decodes input (tbyte) into output (usigned long).
 *
 * @param {input} tbytes.
 *
 ...
 *
 */
void MD5::decode(const tbyte* input, bit32* output, size_t length) {
  for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
