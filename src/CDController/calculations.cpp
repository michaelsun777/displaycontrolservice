...
void ltrOutputs_dst(const list<shared_ptr<Output>> &outputs,const shared_ptr<Mode> &pMode,const list<shared_ptr<Pos>> & outputsPos) {
    int xpos = 0;
    int ypos = 0;
    for (const auto &output : outputs) {
        ...
        if (output->desiredActive) {
            ...
            // set the desired mode to optimal
            output->desiredMode = pMode;

            for (const auto &pos : outputsPos)
            {
                if (!pos->outName.empty() && strcasecmp(pos->outName.c_str(),output->name.c_str()) == 0)
                {
                    xpos = pos->x;
                    ypos = pos->y;
                    output->desiredPos = make_shared<Pos>(xpos, ypos);
...
void ltrOutputs_ex(const list<shared_ptr<Output>> &outputs,const shared_ptr<Mode> &pMode) {
    int xpos = 0;
    int ypos = 0;
    for (const auto &output : outputs) {
...
bool checkOutputs_dst(const std::list<std::shared_ptr<Output>> &outputs,const std::list<std::shared_ptr<Pos>> & outputsPos,int width,int height)
{
    for (const auto &output : outputs) 
    {
        ...
        if (output->desiredActive) {
            ...
            // set the desired mode to optimal
            //output->desiredMode = pMode;
            if(output->currentMode->width != width || output->currentMode->height != height)
            {
                return false;
            }
            ...
            for (const auto &pos : outputsPos)
            {
                if (!pos->outName.empty() && strcasecmp(pos->outName.c_str(),output->name.c_str()) == 0)
                {                    
                    if(output->currentPos->x != pos->x || output->currentPos->y != pos->y)
                    {
                        return false;
                    }
                }                
